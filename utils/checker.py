...
def getSignatureArea(img: np.ndarray)->np.ndarray:
    return img[10:150, 1220:-50]

def evaluate(rect_contours: list, no_double = False):
    eval = []
    for rect in rect_contours:
        
        ret, rect = cv.threshold(rect[30:-30], THRESHOLD_VAL, 255, cv.THRESH_BINARY_INV)
